...
- **AI 工作流** — 可视化节点编辑器，支持预设模板、实时执行、失败重试、强制停止（终止卡住节点）、文本/图片/视频/音频节点
- **模型管理** — 多供应商模型注册中心，支持渠道管理、路由配置和费用统计
- **角色流水线** — 主模板 → 方向变体 → 动画精灵 → 精灵表
- **图片处理** — 背景移除、精灵对齐、精灵表打包（4 步流程：切分→编辑→导出→MAGIC 超分）、视频帧提取（关键帧选取 + 内容缩放）、图片网格合并
...
| **SSE 推送** | sse-starlette | 工作流执行进度实时推送 |
...
│   │   ├── workflow_helper.py # 工作流 CRUD + 执行 + 强制停止
│   │   ├── routers/          # FastAPI 路由
│   │   │   ├── workflow_router.py  # 工作流 CRUD + 运行 + 强制停止 + 预设端点
...
| **ExtractFrames** | 处理 | 从视频中提取帧，支持关键帧选取（循环检测 / 均匀 / 多样性）+ 内容缩放（滑块 1%–200%） |
| **PackSpritesheet** | 处理 | 将帧打包为精灵表 |
| **GridMerge** | 处理 | 将多张图片合并为可配置网格（列 × 行） |
...
| **VideoFrameExtract** | 从视频提取帧，支持关键帧选取（循环检测、均匀采样、多样性采样） |
| **ImageGridMerge** | 将多张图片合并为网格布局，支持配置列数、行数和间距 |

自定义组件是独立的插件，拥有独立的 schema、凭据和执行逻辑。通过 [ComponentRegistry](src/spriteflow/components/registry.py) 集成到工作流引擎，并通过 [ComponentsPage](web/src/pages/components/ComponentsPage.tsx) 界面管理。单节点执行会自动通过 handle-to-parameter 边注入解析上游节点输出。

### 精灵工具 (纯前端)

| 工具 | 说明 |
|------|------|
| **精灵表工具** | 4 步流程：切分 → 编辑 → 导出 → **MAGIC 超分辨率处理**（Real-ESRGAN 4x 动漫超分 + 1/2、1/4、1/8 透明 PNG 变体 + 合成 PNG 导出） |
| **视频帧工具** | 从视频提取帧 → 关键帧选取（循环/均匀/多样性）→ 批量编辑（翻转、偏移、**内容缩放**）→ 导出 PNG/GIF/ZIP |
| **图片编辑器** | 即梦 AI 修复、背景移除、像素级编辑 |

### 近期 UI 改进

- **滑块拖拽**：数字输入改为可拖拽滑块（列数/行数/合成帧数）
- **统一 Emoji 图标**：用平台原生 emoji 替换框架图标组件，确保跨平台渲染一致
- **资源处理分组**：步骤 3 导出按钮归类为「资源处理」标签，按钮尺寸统一

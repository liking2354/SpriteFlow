...
const NodeFlow = ({ initialNodeSchemas, initialWorkflowData }) => {
  ...
  const initialState = useMemo(() => {
    console.log("[NodeFlow] processWorkflowData called with:", {
      hasWorkflowData: !!initialWorkflowData,
      hasCategories: !!initialNodeSchemas?.categories,
      id,
    });
  ...
  // Memoize fitView callback to prevent re-creating it on every render,
  // which would cause xyflow's StoreUpdater to set fitViewQueued to a truthy value
  // and trigger unwanted fitView during rapid zoom-out operations
  ...
  const loadPreset = async (preset) => {
    ...
    if (preset.id === "empty-workflow") return;
    try {
      const res = await axios.get(`/api/workflow/presets/${preset.id}`);
  ...
  useEffect(() => {
    axios.get("/api/workflow/presets")
      .then(res => {
        const list = res.data || [];
        setApiPresets(list);
      })
      .catch(() => {
        // Fallback: only empty-workflow
        setApiPresets([
          { id: "empty-workflow", title: "Empty Workflow", description: "", icon: "plus", image: "", node_count: 0, edge_count: 0 }
        ]);
      });
  }, []);

  // Fetch available text models for ChatWidget model selector
  useEffect(() => {
    axios.get("/api/model-manager/models?category=text&limit=50")
      .then(res => {
        const models = res.data?.items || [];
        setAvailableTextModels(models);
        // If no model selected, default to the one marked is_default
        if (!selectedModel) {
      ...
      .catch(err => console.warn("Failed to fetch text models:", err));
  }, []);
  ...
  // Close settings popup on click outside
  useEffect(() => {
      ...
      try {
        ...
        if (status === "completed") {
          ...
          const newAgentMessage = {
            role: "agent",
            content: message || t('chat.tasksComplete'),
        ...
        const errorMessage = {
          role: "agent",
          content: t('chat.workflowUpdateError'),
          timestamp: new Date().toISOString()
        };
          ...
          Object.entries(nodesStatus).forEach(([id, runs]) => {
            ...
            if (status === "succeeded" || status === "completed") {
              ...
              setNodes((prevNodes) => prevNodes.map(n => {
                if (n.id === id) {
                  return { ...n, data: { ...n.data, isLoading: false, errorMsg: t('editor.generationFailed') } };
                }
                return n;
              }));
            }
  ...
  const handlePublishWorkflow = async () => {
    if (!interactionMode || publishingRef.current) return;
    ...
    try {
      ...
      setIsRunning(0);
      toast.success(response.data.publish ? t('editor.publishedSuccessfully') : t('editor.unpublishedSuccessfully'));
  ...
  const handleTemplatePublish = async () => {
    ...
    try {
      ...
      setIsRunning(0);
      toast.success(is_template ? t('editor.publishedSuccessfully') : t('editor.unpublishedSuccessfully'));
  ...
  const handleCategorySave = async () => {
    if (!workflowId) {
      toast.error(t('editor.workflowIdNotFound'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('editor.categoryUpdated'));
    } catch (error) {
  ...
  const duplicateNode = useCallback((nodeId) => {
    ...
    toast.success(t('editor.duplicatedNode', { sourceId: nodeId, targetId: newNodeId }));
  }, [nodes, setNodes]);
  ...
  const getModelsForNode = (node) => {
    ...
    const mapModels = (modelsMap) =>
      modelsMap ? Object.entries(modelsMap).map(([id, model]) => ({
        ...model,
        id,
        name: SPECIAL_MODEL_KEYS[id] ? t(SPECIAL_MODEL_KEYS[id]) : (SPECIAL_MODEL_NAMES[id] || model?.name || formatName(id))
      })) : [];
  ...
  return (
    <div tabIndex={0} onKeyDown={onKeyDown} className="flex h-full w-full relative">
      {isRestoring && (
      ...
      <div className="flex items-center justify-center absolute top-0 z-20 bg-[#151618] w-full py-1.5 border-b border-gray-800">
        <div className="flex items-center justify-between w-full max-w-[95%] sm:max-w-[90%] lg:max-w-[80%] overflow-x-auto">
          <div className="flex items-center gap-1.5 w-[35%]">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm outline-none text-[#adacaa] hover:text-white cursor-pointer bg-transparent max-w-[90%]"
            >
              <span className="truncate block w-full">{workflowName ? workflowName : t('editor.untitled')}</span> <FaRegEdit size={12} />
            </button>
          </div>
            ...
            {interactionMode && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 border border-gray-600/70 bg-white text-black text-sm rounded-full hover:bg-black hover:text-white transition-colors"
                >
                  <MdSave size={14} /> {t('common.save')}
                </button>
                <div className="relative" ref={settingsRef}>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1 border border-gray-600/70 bg-white text-black text-sm rounded-full hover:bg-black hover:text-white transition-colors"
                  >
                    <FaToolbox size={12} /> {t('editor.settings')} <FaAngleDown size={10} className={`transition-transform duration-300 ${isSettingsOpen ? "rotate-180" : ""}`} />
                  </button>
                    ...
                    <div className="absolute right-0 mt-2 w-48 bg-[#1b1e23] border border-gray-700 rounded-lg shadow-xl z-50 overflow-hidden">
                      {template.showTemplateBtn && (
                        ...
                        >
                          ...
                          <span>{template.isPublishedTemplate ? t('editor.undoTemplate') : t('editor.makeTemplate')}</span>
                        </button>
                      )}
                      <button
                        type="button"
            ...
            {interactionMode ? (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 border border-gray-600/70 bg-white text-black text-sm rounded-full group cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:bg-black hover:text-white"
                >
                  {isRunning === 2 ? (
                    <>
                      <div className="w-3.5 h-3.5 border-2 border-t-transparent border-black group-hover:border-white group-hover:border-t-transparent rounded-full animate-spin"></div> {t('editor.publishing')}
                    </>
                  ) : (
                    <>
                      <FaTelegramPlane size={14} /> {publishWorkflow ? t('editor.unpublish') : t('editor.publish')}
                    </>
                  )}
                </button>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 border border-gray-600/70 bg-blue-500 text-white text-sm rounded-full font-semibold group cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:bg-black hover:text-white whitespace-nowrap"
                >
                  {isRunning === 1 ? (
                    <>
                      <div className="w-3.5 h-3.5 border-2 border-t-transparent border-black group-hover:border-white group-hover:border-t-transparent rounded-full animate-spin"></div> {t('editor.running')}
                    </>
                  ) : (
                    <>
                      <FaPlay size={14} /> {t('editor.runAll')} {parseFloat(totalWorkflowCost) > 0 && `($${totalWorkflowCost})`}
                    </>
                  )}
                </button>
              ...
              >
                {isRunning === 3 ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-t-transparent border-black group-hover:border-white group-hover:border-t-transparent rounded-full animate-spin"></div> {t('editor.duplicating')}
                  </>
                ) : (
                  <>
                    <IoDuplicateOutline size={14} /> {t('common.duplicate')}
                  </>
                )}
              </button>
            )}
      ...
      <div className={`absolute left-4 self-center z-20 flex flex-col gap-2 bg-[#151618] p-1 rounded-full border border-gray-700 shadow-xl ${isRestoring && "hidden"}`}>
        <button
          type="button"
          suppressHydrationWarning={true}
          onClick={() => toast.error(t('editor.cannotEdit'))}
          title={t('workflow.lockHint')}
        ...
        >
          ...
          {dropDown === 4 && (
            <div className="absolute left-14 top-0 bg-[#1b1e23] border border-gray-700 p-3 rounded-lg flex flex-col gap-2 w-52">
              <h3 className="w-full text-center text-sm text-gray-300">{t('editor.utilityNode')}</h3>
              <div className="flex flex-col gap-2 w-full">
                ...
                >
                  <TbArrowMerge className="rotate-90" /> <span className="text-xs font-medium">{t('editor.promptConcatenator')}</span>
                </button>
                <button
                  type="button"
                  suppressHydrationWarning={true}
                  ...
                  className="flex gap-2 justify-center items-center py-3 px-4 text-white cursor-pointer bg-[#2c3037] rounded hover:bg-[#212326]"
                >
                  <TbArrowMerge className="rotate-90" /> <span className="text-xs font-medium">{t('editor.videoCombiner')}</span>
                </button>
              </div>
            </div>
          )}
        ...
        <button
          ...
          title={isPanning ? t('workflow.selectMode') : t('workflow.panMode')}
          className={`p-3 rounded-full cursor-pointer outline-none active:bg-gray-600 transition ${isPanning ? "bg-white text-black" : "text-gray-300 hover:bg-[#1b1e23] hover:text-white"}`}
        >
          {isPanning ? <FaRegHand size={18} /> : <LuMousePointer2 size={18} />}
        </button>
      </div>
      <div className="z-10 w-full h-full">
        <ReactFlow
          ...
          nodesDraggable={interactionMode && !isPanning}
          ...
          selectionMode={!isPanning ? "partial" : null}
      ...
      {selectedNode && !["concatNode"].includes(selectedNode.type) && (
        <div className="absolute right-2 top-16 z-50 w-80 h-full max-h-[90%] bg-[#09090b]/80 backdrop-blur-xl border border-white/20 rounded-2xl flex transition-all duration-300 ease-in-out shadow-2xl">
          ...
          <div className="flex flex-col gap-4 h-full w-full">
            <h3 className="text-base font-semibold text-center text-white mt-6 tracking-tight">{t('editor.properties')}</h3>
            ...
            <div className="flex flex-col gap-4 w-full h-full overflow-y-auto px-4 custom-scrollbar-thin">
              <div className="flex flex-col gap-4 w-full h-full">
                <div
                  ...
                  tabIndex={0}
                >
                  <label className="text-[10px] font-bold text-zinc-500 text-start px-1">{t('editor.model')}</label>
                  <button
                    type="button"
                    suppressHydrationWarning={true}
                ...
                {selectedNode?.data?.selectedModel ? (
                  (() => {
                    ...
                    return selectedNode?.data?.loading === 1 ? (
                      <div className="flex flex-col items-center justify-center gap-2 h-full w-full">
                        ...
                        <span className="text-xs text-white">{t('editor.fetchingModel')}</span>
                      </div>
                      ...
                      <div className="flex flex-col gap-2 w-full h-full relative pt-2">
                        ...
                        >
                          {selectedNode?.data?.loading === 1 ? (
                            <><div className="w-3 h-3 rounded-full border border-t-transparent group-hover:border-t-transparent border-black group-hover:border-white animate-spin"></div>{t('editor.generating')}</>
                          ) : (
                            <>{t('editor.fetchModel')}</>
                          )}
                        </button>
                  ...
                  <div className="text-center py-8">
                    <p className="text-sm text-gray-400">{t('editor.pleaseSelectModel')}</p>
                  </div>
                )}
              </div>
            </div>
            <div className="p-4 flex flex-col gap-3">
              ...
              <label className="flex items-center justify-between cursor-pointer group">
                <span className="text-xs text-gray-300 font-medium">{t('editor.markAsOutput')}</span>
              ...
              {!selectedNode?.data?.selectedModel?.id?.includes("passthrough") && (
                ...
                >
                  {loadingNodes[selectedNode.id] ? (
                    <><div className="w-4 h-4 rounded-full border-2 border-white/20 border-t-white animate-spin"></div>{t('editor.generating')}</>
                  ) : (
                    <>
                      <FaPlay size={16} /> 
                      {t('common.generate')}
                      {generationCost !== null && (
                        <span className="text-xs font-medium">
                          {isRefreshingCost ? (
                            ...
                            generationCost === 0 ? t('common.free') : `$${generationCost}`
                          )}
                        </span>
                      )}
                    </>
      ...
      >
        <div className="bg-[#242629] rounded-lg p-4 w-72 shadow-lg flex flex-col gap-4" onClick={(e) => e.stopPropagation()}>
          <h3 className="text-base text-center font-semibold text-white">{t('editor.saveWorkflow')}</h3>
          <div className="flex flex-col gap-2 w-full">
            <label className="text-xs text-start text-gray-300">{t('editor.workflowName')}</label>
            <input
              type="text"
              value={workflowName}
              autoFocus
              onChange={(e) => setWorkflowName(e.target.value)}
              placeholder={t('editor.enterWorkflowName')}
        ...
        <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
          <div className="pointer-events-auto flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-300 transform scale-90 md:scale-100 overflow-y-auto custom-scrollbar max-w-[90%] max-h-[80%] p-10">
            <div className="flex flex-col items-center gap-2 bg-black/40 backdrop-blur-md px-6 py-3 rounded-lg border border-white/10 shadow-xl">
              <h2 className="text-xl font-semibold text-white tracking-tight">{t('editor.selectWorkflow')}</h2>
              <p className="text-xs text-gray-400 font-medium uppercase tracking-widest">{t('editor.orStartFromScratch')}</p>
            </div>
                ...
                >
                  <div className="z-10 p-2 bg-[#242629] border-b border-gray-700 flex items-center px-3 justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-gray-300 uppercase tracking-wider">{preset.id === "empty-workflow" ? t('editor.new') : t('editor.preset')}</span>
                    </div>
            ...
            >
              {t('editor.dismissEnterEmptyCanvas')}
            </button>
          </div>
        </div>
      )}
      ...
      {isCategoryPopupOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-[#1b1e23] border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-white mb-4">{t('editor.editWorkflowCategory')}</h3>
              <div className="space-y-4">
                <div className="flex flex-col gap-2">
                  <label className="text-xs text-gray-400 uppercase tracking-wider">{t('editor.categoryName')}</label>
                  <input
                    type="text"
                    value={categoryInput}
                    onChange={(e) => setCategoryInput(e.target.value)}
                    placeholder={t('editor.enterCategory')}

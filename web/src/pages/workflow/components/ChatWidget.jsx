        ...
        >
          {copied ? (
            <>
              ...
              <span className="text-emerald-400 uppercase tracking-wider">{t('common.copied')}</span>
            </>
          ) : (
            <>
              <FaRegCopy size={12} />
              <span className="uppercase tracking-wider">{t('common.copy')}</span>
            </>
          )}
        </button>
      </div>
...
const ChatWidget = ({ isOpen, toggleChat, messages, onSendMessage, isLoading, onClearHistory, selectedModel, onModelChange, availableModels }) => {
  ...
  const loadingTexts = [t('chat.thinking'), t('chat.analyzing'), t('chat.generating'), t('chat.refining'), t('chat.processing'), t('chat.running')];
          ...
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-purple-600/20 to-blue-600/20 border-b border-white/10">
            ...
            <div className="flex items-center gap-2">
              ...
              {availableModels && availableModels.length > 0 && (
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-white/5 text-gray-300 rounded-lg hover:bg-white/10 border border-white/10 transition-colors"
                    title={t('chat.selectModel') || 'Select Model'}
                  >
                    <span className="max-w-[80px] truncate">
                      {selectedModel ? availableModels.find(m => m.model_id === selectedModel)?.name || selectedModel : (t('chat.defaultModel') || 'Default')}
                    </span>
                    <FaChevronDown size={10} className={`transition-transform ${modelDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {modelDropdownOpen && (
                    <div className="absolute right-0 top-full mt-1 w-56 bg-[#1A1F2B] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
                      <div className="py-1 max-h-64 overflow-y-auto custom-scrollbar">
                        <button
                          type="button"
                          suppressHydrationWarning={true}
                          onClick={() => { onModelChange(''); setModelDropdownOpen(false); }}
                          className={`w-full text-left px-4 py-2 text-xs hover:bg-white/5 transition-colors flex items-center justify-between ${!selectedModel ? 'text-blue-400' : 'text-gray-300'}`}
                        >
                          <span>{t('chat.defaultModel') || 'Default (Model Manager)'}</span>
                          {!selectedModel && <span className="text-blue-400">✓</span>}
                        </button>
                        {availableModels.map((m) => (
                          <button
                            ...
                            type="button"
                            suppressHydrationWarning={true}
                            onClick={() => { onModelChange(m.model_id); setModelDropdownOpen(false); }}
                            className={`w-full text-left px-4 py-2 text-xs hover:bg-white/5 transition-colors flex items-center justify-between ${selectedModel === m.model_id ? 'text-blue-400' : 'text-gray-300'}`}
                          >
                            <div className="flex items-center gap-2">
                              <span>{m.name}</span>
                              {m.is_default && (
                                <span className="px-1.5 py-0.5 text-[9px] bg-amber-500/20 text-amber-400 rounded-full font-medium">★</span>
                              )}
                            </div>
                            {selectedModel === m.model_id && <span className="text-blue-400">✓</span>}
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <button
                ...
                title={isWide ? t('chat.narrowView') : t('chat.wideView')}
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-6 text-center p-6 h-full">
                <div className="flex flex-col items-center gap-2 text-gray-500">
                  ...
                  <h4 className="text-lg font-semibold text-white mt-2">{t('chat.welcome')}</h4>
                  <p className="text-sm max-w-[250px]">{t('chat.howCanIHelp')}</p>
                </div>
                      ...
                      >
                        ...
                        {msg.suggestions && msg.suggestions.length > 0 && (
                          <div className="flex flex-col gap-2">
                            <p className="text-xs font-medium text-gray-400">{t('chat.suggestedActions')}</p>
